<section class="our_service_area">
    <div class="container">
        <div class="row our_service_inner">
            <div class="col-md-3 col-sm-6">
                <div class="our_service_first">
                    <h3>Resort Services</h3>
                    <p>quia non numquam eius modi tempora incidunt ut labore et dolore magnam aliquam quaerat voluptatem. Ut enim ad minima veniam.</p>
                    <a class="all_s_btn" href="{{route('page.about_us')}}/#sec_our_services">view all services</a>
                    <i class="fa fa-sport"></i>
                </div>
            </div>
            <div class="col-md-3 col-sm-6">
                <div class="our_service_item">
                    <img
                        class="img-fluid"
                        style="max-width:256px; width:27%;"
                        src="{{asset('images/services/microphone_flaticon.png')}}" alt="">
                    <h4>Karaoke</h4>
                    <p>Incidunt ut labore et dolore magnam aliquam quaerat volup tatem. Utad minima.</p>
                </div>
            </div>
            <div class="col-md-3 col-sm-6">
                <div class="our_service_item">
                    <i class="flaticon-toaster"></i>
                    <h4>Buffet Breakfast</h4>
                    <p>Incidunt ut labore et dolore magnam aliquam quaerat volup tatem. Utad minima.</p>
                </div>
            </div>
            <div class="col-md-3 col-sm-6">
                <div class="our_service_item">
                    <i class="flaticon-taxi"></i>
                    <h4>Transportation Service</h4>
                    <p>If you require local transportation, a transportation service can be arranged for you at the Reception Tour Desk.</p>
                </div>
            </div>
        </div>
    </div>
</section>
